...
INSTRUCTIONS = (
    ...
    "across a source is a warning, not a failure; the client-engine "
    "simulation is a deliberately bounded sample (it walks at most page_cap "
    "listing pages politely), so judge it on within-sample agreement with "
    "the crawl and on pagination evidence (paginated, and "
    "continuation_after_sample when more pages remain) — full-lot coverage "
    "is the crawl's responsibility, and a simulation covering fewer vehicles "
    "than the lot is NOT a defect by itself; a config field the factory "
    "reconciled away (fields_reconciled) is supplied by the extension's "
    "fixed detail-page pass, so its absence from simulated listing rows is "
